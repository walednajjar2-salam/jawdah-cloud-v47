...
function drawLinePro(id, arr){ const c=$('#'+id); if(!c) return; const [g,w,h]=prepCanvas(c); g.clearRect(0,0,w,h); const vals=[...arr,1], max=Math.max(...vals)*1.25; g.strokeStyle='rgba(255,255,255,.08)'; for(let i=0;i<4;i++){let y=20+i*(h-50)/3;g.beginPath();g.moveTo(20,y);g.lineTo(w-20,y);g.stroke();} g.beginPath(); arr.forEach((v,i)=>{ const x=24+i*(w-48)/(arr.length-1||1), y=h-28-(v/max)*(h-58); i?g.lineTo(x,y):g.moveTo(x,y); }); g.strokeStyle='#c9a227'; g.lineWidth=3; g.shadowColor='rgba(201,162,39,.35)'; g.shadowBlur=8; g.stroke(); g.shadowBlur=0; arr.forEach((v,i)=>{ const x=24+i*(w-48)/(arr.length-1||1), y=h-28-(v/max)*(h-58); g.beginPath(); g.fillStyle='#d4af37'; g.arc(x,y,4,0,Math.PI*2); g.fill(); }); }
function drawDonutPro(id,p){ const c=$('#'+id); if(!c) return; const [g,w,h]=prepCanvas(c); g.clearRect(0,0,w,h); const x=w/2,y=h/2,r=Math.min(w,h)/2.8; g.lineWidth=16; g.strokeStyle='rgba(255,255,255,.12)'; g.beginPath(); g.arc(x,y,r,0,Math.PI*2); g.stroke(); const pct=Math.max(0,Math.min(100,Number(p||0))); g.strokeStyle='#5b9aa0'; g.beginPath(); g.arc(x,y,r,-Math.PI/2,-Math.PI/2+Math.PI*2*(pct/100)); g.stroke(); if(pct<100){ g.strokeStyle='#c9a66b'; g.beginPath(); g.arc(x,y,r,-Math.PI/2+Math.PI*2*(pct/100),-Math.PI/2+Math.PI*2); g.stroke(); } }
function drawBarPro(id,arr){ const c=$('#'+id); if(!c) return; const [g,w,h]=prepCanvas(c); g.clearRect(0,0,w,h); const max=Math.max(...arr,1)*1.25, colors=['#5b9aa0','#7eb8d4','#c9a66b','#6db894','#a898c9','#d48989']; const bw=(w-50)/arr.length*.55; arr.forEach((v,i)=>{ const x=24+i*(w-50)/arr.length+8, bh=(v/max)*(h-36); g.fillStyle=colors[i%colors.length]; g.fillRect(x,h-22-bh,bw,bh); }); }
