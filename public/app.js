...
const SECTION_TITLES = {
  dashboard:'لوحة التحكم',properties:'المشاريع',tasks:'المهام',clients:'العملاء',contracts:'العقود',
  revenues:'الإيرادات',invoices:'المدفوعات','admin-expenses':'المصروفات',maintenance:'الصيانة',
  reports:'التقارير',messages:'الرسائل',production:'المتابعة',timeline:'الجدول الزمني',
...
function friendlyMsg(e, fallback='تعذر إتمام العملية'){
  ...
  if(!raw||raw==='Request failed'||raw==='Invalid response'||/failed to fetch|network/i.test(raw)) return fallback;
  if(/403|forbidden|permission|صلاح/i.test(raw)) return 'لا تملك صلاحية لهذه العملية';
  if(/401|unauthorized|token|session/i.test(raw)) return 'انتهت الجلسة، سجّل الدخول مجدداً';
  if(/404|not found|لم يتم/i.test(raw)) return 'العنصر غير موجود';
  if(/duplicate|unique|exists/i.test(raw)) return 'البيانات مسجلة مسبقاً';
  if(/[\u0600-\u06FF]/.test(raw)) return raw.replace(/^(error|detail)[:\s]*/i,'');
  return fallback;
}
...
const api = async (path, opts={}) => {
  ...
  if(!res.ok || data.ok === false){ const err=new Error(data.error||data.detail||''); err.status=res.status; throw err; }
  return data;
};
...
function isDashboardShell(){ return /app\.html$/i.test(location.pathname); }
...
async function logout(){ try{await api('logout',{method:'POST'});}catch(e){} localStorage.removeItem('jawdah_cloud_token'); Jawdah.token=''; location.href='/'; }
...
function showLoginShell(){
  document.body.classList.add('login-ultra','saas-login','enterprise-vision');
...
async function checkSession(){
  ...
  if(!Jawdah.token){
    if(isDashboardShell()){ location.replace('/?login=1'); return; }
    showLoginShell();
    return;
  }
  try{
...
async function loadAll(){
  ...
  try{
    ...
    try{
      ...
      Jawdah.uiPermissions={sections:perm.sections||[],kpis:perm.kpis||[]};
    }catch(e){ Jawdah.uiPermissions=null; }
    applyUserHeader();
    applyFieldMode();
    buildNav(); renderSidebarUser(); syncOpsBar(); syncFabDock(); renderAll(); showSection(Jawdah.activeSection||'dashboard'); ensureEnglishDigits();
    connectLiveStream();
...
function showSection(id){
  ...
  const needsSkeleton=!['dashboard','tasks','messages','timeline'].includes(resolved);
...
function renderDashRenewalStrip(){
  ...
  if(!queue.length){ host.innerHTML=''; host.className='saas-renewal-strip'; return; }
...
function renderDashboard(){
  ...
  if(projGrid) projGrid.innerHTML=props.slice(0,12).map(p=>{
    ...
    return `<article class="saas-glass saas-project-card" onclick="showSection('properties')"><div class="proj-top"><h4>${propertyLabel(p)}</h4><span class="saas-status ${cls}">${p.status||'—'}</span></div><div class="saas-prog-bar"><i style="width:${prog}%"></i></div><div class="saas-proj-meta"><span>💵 ${money(p.price||0)}</span><span>📍 ${p.location||'Oman'}</span>${c?`<span>📅 ${c.end_date}</span>`:''}</div></article>`;
...
async function submitPayment(){ try{ await api('pay_invoice',{method:'POST',body:JSON.stringify({invoice_id:val('payInvoiceId'),amount:num('payAmount'),method:val('payMethod'),note:val('payNote')})}); closeModal('paymentModal'); toast('تم التحصيل وتحديث الحسابات'); await loadAll(); }catch(e){toastErr(e)} }
...
async function runQA(){
  ...
  }catch(e){ html+=`<p class="mini">فحص النسخ الاحتياطي: جاري التحديث</p>`; }
...
function applyLiveEvent(payload){
  ...
  if(bell && payload.type==='kpis'){
    bell.classList.toggle('hidden', !(Number(k.overdue||0)>0 || Number(k.expiring||0)>0));
  }
