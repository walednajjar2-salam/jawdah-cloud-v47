(function () {
  ...
  function renderBridge() {
    ...
    if (!bridge) {
      ...
      dash.insertBefore(bridge, dash.firstElementChild);
    }
  ...
  window.printInvoice = function (id) {
    const invoice = data("invoices").find((item) => item.id === id);
    if (!invoice) return;
    const contract = data("contracts").find((c) => c.id === invoice.contract_id) || {};
    const payments = invoicePayments(id);
    const status = invoiceStatusMeta(invoice);
    window.Jawdah = window.Jawdah || {};
    window.Jawdah.invoiceForPrint = invoice;
    focusPrintModal();
    q("#invoicePreview").innerHTML = `<div class="invoice-paper lq-print-paper">
      <div class="head lq-print-head"><div><img src="assets/brand-logo-gold.png" alt="Launch Quality"><h1>TAX INVOICE</h1><h2>Launch Quality LLC</h2><p>Real Estate & Hospitality Management<br>C.R. 1466316 - Sultanate of Oman</p></div><div><h2>${esc(invoice.invoice_no)}</h2><p>Issue: ${esc(invoice.issue_date)}<br>Due: ${esc(invoice.due_date)}<br>Status: ${status.label}</p></div></div>
      <div class="grid lq-print-grid"><div><h3>Client</h3><p>${esc(contractClientName(invoice.client_id))}</p></div><div><h3>Contract / Property</h3><p>${esc(invoiceContractLabel(invoice.contract_id))}<br>${esc(contractPropertyLabel(invoice.property_id))}</p></div></div>
      <table><thead><tr><th>Description</th><th>Amount</th></tr></thead><tbody><tr><td>${esc(invoice.description)}</td><td>${safeMoney(invoice.amount)}</td></tr></tbody></table>
      <div class="lq-print-totals"><p>Total <b>${safeMoney(invoice.amount)}</b></p><p>Paid <b>${safeMoney(invoice.paid_amount)}</b></p><p>Remaining <b>${safeMoney(invoiceRemaining(invoice))}</b></p></div>
      <p class="mini">Payments recorded: ${safeFmt(payments.length)}</p>
      <div class="lq-print-sign"><span>Prepared By</span><span>Client Signature</span><span>Company Stamp</span></div>
    </div>`;
  ...
  window.printReceipt = function (invoiceId, paymentId = "") {
    ...
    focusPrintModal();
    q("#invoicePreview").innerHTML = `<div class="invoice-paper lq-print-paper">
      <div class="head lq-print-head"><div><img src="assets/brand-logo-gold.png" alt="Launch Quality"><h1>RECEIPT VOUCHER</h1><h2>Launch Quality LLC</h2><p>Real Estate & Hospitality Management</p></div><div><h2>${esc(invoice.invoice_no)}</h2><p>Receipt: ${esc(payment.id)}<br>Date: ${esc(payment.payment_date)}<br>Method: ${esc(payment.method)}</p></div></div>
      <div class="grid lq-print-grid"><div><h3>Received From</h3><p>${esc(contractClientName(invoice.client_id))}</p></div><div><h3>Against Invoice</h3><p>${esc(invoice.invoice_no)}<br>${esc(invoice.description)}</p></div></div>
      <div class="lq-receipt-amount"><span>Amount Received</span><strong>${safeMoney(payment.amount)}</strong></div>
      <p>${esc(payment.note || "Invoice payment")}</p>
      <div class="lq-print-sign"><span>Received By</span><span>Client Signature</span><span>Company Stamp</span></div>
    </div>`;
    if (typeof openModal === "function") openModal("invoiceModal");
  };
