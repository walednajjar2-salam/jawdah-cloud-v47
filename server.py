...
from auto_backup import build_auto_backup_service
...
def init_db() -> None:
    ...
    with connect() as db:
        ...
        if ADMIN_PASSWORD:
            ensure_user(db, "admin", "مدير النظام", "admin", ADMIN_PASSWORD, sync_password=True)
...
def seed_if_empty(db: sqlite3.Connection) -> None:
    if db.execute("SELECT COUNT(*) FROM users").fetchone()[0] == 0:
        ...
        if ADMIN_PASSWORD:
            defaults.insert(0, ("admin", "System Admin", "admin", ADMIN_PASSWORD))
...
class JawdahHandler(BaseHTTPRequestHandler):
    ...
    def serve_static(self, path: str) -> None:
        ...
        if str(full).startswith(str(public_root)) and full.exists() and not full.is_dir():
            ...
            self.send_header("Content-Length", str(len(raw)))
            if full.suffix == ".html" or path in ("/", "/index.html"):
                self.send_header("Cache-Control", "no-cache, no-store, must-revalidate")
            elif full.suffix in {".js", ".css"}:
        ...
        try:
            with connect() as db:
                ...
                if parts[0] == "health" and method == "GET":
                    ...
                    if AUTO_BACKUP:
                        payload["auto_backup"] = AUTO_BACKUP.status()
                ...
                if parts[0] == "backup" and method == "GET" and len(parts) >= 2 and parts[1] == "auto":
                    user = self.require_user(db, "backup:export")
                    return None if not user else self.api_backup_auto()
                if parts[0] == "backup" and method == "POST" and len(parts) >= 2 and parts[1] == "run":
                    user = self.require_user(db, "admin")
                    return None if not user else self.api_backup_run(user)
    ...
    def api_login(self, db: sqlite3.Connection) -> None:
        ...
        if username.lower() == "admin" and not ADMIN_PASSWORD:
    ...
    def api_backup(self, db: sqlite3.Connection) -> None:
        if AUTO_BACKUP:
            payload = AUTO_BACKUP.build_export_payload(db)
        else:
    ...
    def api_backup_auto(self) -> None:
        if not AUTO_BACKUP:
            self.send_json({"ok": False, "error": "Automatic backup is not configured"}, 503)
            return
        self.send_json({"ok": True, "auto_backup": AUTO_BACKUP.status()})

    def api_backup_run(self, user: Dict[str, Any]) -> None:
        if not AUTO_BACKUP:
            self.send_json({"ok": False, "error": "Automatic backup is not configured"}, 503)
            return
        result = AUTO_BACKUP.run_now(reason=f"manual:{user.get('username') or 'admin'}")
        adb = connect()
        try:
            audit(adb, user, "backup", "auto_backup", result.get("archive"), "Manual automatic backup run")
            adb.commit()
        finally:
            adb.close()
        self.send_json({"ok": True, "result": result, "auto_backup": AUTO_BACKUP.status()})
...
def main() -> None:
    global AUTO_BACKUP
    init_db()
    AUTO_BACKUP = build_auto_backup_service(
        data_dir=DATA_DIR,
        db_path=DB_PATH,
